...
-(void)initUI{
    ...
    //背景竖线
    CGFloat barHeight = CGRectGetHeight(self.frame) * 0.8;
    CGFloat barY = (CGRectGetHeight(self.frame) - barHeight) / 2.0;
    _verticalBar = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.frame) / 2.0, barY + 10, 1.0, barHeight)];
    _verticalBar.backgroundColor = [[UIColor lightGrayColor] colorWithAlphaComponent:0.7];
    [_backView addSubview:_verticalBar];
    
    //背景左边竖线
    _leftBar = [[UIImageView alloc] initWithFrame:CGRectMake(0, barY, 25, barHeight)];
    _leftBar.backgroundColor = [UIColor whiteColor];
    [_leftBar setImage:[UIImage imageNamed:@"icon_left_bar.png"]];
    [_backView addSubview:_leftBar];
    
    //背景右边竖线
    _rightBar = [[UIImageView alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.frame) - 25, barY, 25, barHeight)];
    _rightBar.backgroundColor = [UIColor whiteColor];
    [_rightBar setImage:[UIImage imageNamed:@"icon_right_bar.png"]];
    [_backView addSubview:_rightBar];
    ...
    else if(self.datePickerMode == UIYUDatePickerModeClock)
    {
        if (!PickerView) {
            PickerView = [[UIPickerView alloc]initWithFrame:CGRectMake(0, YU_TOOLBARHIGHT-22, self.frame.size.width, YU_PICKERHIGHR)];
            PickerView.showsSelectionIndicator = YES;
        ...
        [PickerView selectRow:_alarmHour + 48 inComponent:0 animated:NO];
        [PickerView selectRow:_alarmMin + 480 inComponent:1 animated:NO];
        
        self.hourIndex = (_alarmHour + 48) % self.hourArray.count;
        self.minuteIndex = (_alarmMin + 480) % self.minuteArray.count;
    }
    else
    {
...
- (void)setHour:(NSInteger)hour andMinute:(NSInteger)min
{
    _alarmHour = hour;
    _alarmMin = min;
}
...
-(void)revertRow:(NSDate*)date animated:(BOOL)animated{
    ...
    if(self.datePickerMode == UIYUDatePickerModeClock)
    {
//        [PickerView selectRow:12 * 5 inComponent:0 animated:animated];
//        [PickerView selectRow:12 * 5 inComponent:1 animated:animated];
    }
    else
    {
